...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh):nh_(nh), private_nh_(private_nh), tf2_listener_(tf2_buffer_){
  ...
  original_point_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("original_point", 10);
  ...
  const sensor_msgs::PointCloud2::ConstPtr & map_points_msg_ptr)
{
  ...
  ndt_new.setMaximumIterations(max_iterations);
   ...
   pcl::PointCloud<pcl::PointXYZRGBA>::Ptr map_points_ptr(new pcl::PointCloud<pcl::PointXYZRGBA>);
  pcl::fromROSMsg(*map_points_msg_ptr, *map_points_ptr);
  
  // 给点云中的每个点设置颜色，这里设置为白色
  for (auto & point : map_points_ptr->points) {
    point.r = 255;  // 红色通道
    point.g = 255;  // 绿色通道
    point.b = 255;  // 蓝色通道
    point.a = 255;  // Alpha 通道（不透明）
  }

  // 可以在这里发布原始的带颜色的点云，用于可视化
  sensor_msgs::PointCloud2 sensor_points_mapTF_msg;
  pcl::toROSMsg(*map_points_ptr, sensor_points_mapTF_msg);
  sensor_points_mapTF_msg.header.stamp = map_points_msg_ptr->header.stamp;
  sensor_points_mapTF_msg.header.frame_id = map_frame_;
  original_point_pub_.publish(sensor_points_mapTF_msg);

  pcl::PointCloud<pcl::PointXYZ>::Ptr map_points_xyz(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::copyPointCloud(*map_points_ptr, *map_points_xyz);  // 只保留位置数据

  // 设置目标点云
  ndt_new.setInputTarget(map_points_xyz);

  // 进行 NDT 配准
  pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud(new pcl::PointCloud<pcl::PointXYZ>);
  ndt_new.align(*output_cloud, Eigen::Matrix4f::Identity());

  // swap NDT map (这里会锁定 map 的 mutex)
  ndt_map_mtx_.lock();
  ndt_ = ndt_new;
  ndt_map_mtx_.unlock();
}
